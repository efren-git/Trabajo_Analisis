...
tabla_con_marginales <- addmargins(tabla_conts)
tabla_con_marginales
```
...
Si lambda = 0, indica que la variable independiente no es útil para
...
incendio. Tras haber hecho las tablas de contingencia, se ha visto que
cuando el RH es bajo en Sidi, (20%-43.3%) hay un 97% de probabilidad de
incendio. En el resto de casos en Sidi, no se sabe con certeza si va a
haber incendio o no en base al RH.

La tabla de contingencia también muestra que si hay un RH alto en
Bejaia, hay un 82.5% de probabilidad de que no haya un incendio.
...
Ajustamos el modelo exponencial, Y= ae\^(bx), LN(Y) = LN(a) + bx para
Sidi
...
el modelo ajustado es: LN(id) = 1.662319 + 0.004800 \* DMC, obtenemos un
R2 de 4.231%. El modelo es muy malo y no representa bien a los datos.
...
Se observa que la recta de regresión no se ajusta a los datos.
...
El intervalo es (x - 185.3145, x + 185.3145) para cuando se quiera hacer
una predicción
...
### 4.3 Tabla de contingencia Bejaia

Primero haremos las tablas para Bejaia

Transformamos la variable temperatura en una variable categorica ordinal:
Nombrando como temperatura media al intervalo [22-27],
temperatura alta al intervalo (27-33]
temperatura extrema al intervalo (33-37]
```{r}
Bejaia$categoria_temp_Bejaia <- cut(
  Bejaia$Temperature,
  breaks = c(22, 27, 33, 37),   # límites de los intervalos
  labels = c("Media", "Alta", "Extrema"),
  include.lowest = TRUE,
  right = TRUE
)
```
...
```{r}
tabla_absoluta_B <- table(Bejaia$Classes, Bejaia$categoria_temp_Bejaia)
tabla_absoluta_B
tabla_con_marginales_B <- addmargins(tabla_absoluta_B)
tabla_con_marginales_B
```

Realizamos la tabla de contingencia de las frecuencias relativa

```{r}
tabla_relativa_B <- prop.table(tabla_absoluta_B)
tabla_relativa_B
tabla_relativa_marg_B <- addmargins(tabla_relativa_B)
tabla_relativa_marg_B
```

Un 60 % de los dias tienen una temperatura alta.

El 20% de los dias han habido temperaturas extremas e incendios.
El 0.8% de los dias han habido temperaturas mediias e incendios.

Realizamos ahora la tabla de contingencia de la variable temperatura y Classes en Bejaia


```{r}
tabla_condicionada_B <- prop.table(tabla_absoluta_B, margin = 1)*100
tabla_condicionada_B
tabla_condicionada_marg_B <- addmargins(tabla_condicionada_B, margin = 2)
tabla_condicionada_marg_B
```

El 1.69% de los dias con incendio tienen una temperatura media

El 40.67% de los dias con incendios tienen temperaturas extremas

Realizamos un grafico de barras apilado para mostrar los datos

```{r}
tabla_absoluta_graf_B <- data.frame(tabla_absoluta_B)
tabla_absoluta_graf_B
ggplot(tabla_absoluta_graf_B, aes(x = Var1, y = Freq, fill = Var2))+
  geom_bar(stat = "identity", position = "fill")+
  scale_y_continuous(labels = scales::percent)+
  xlab("Clases")+
  ylab("Porcentajes")+
  labs(fill = "intervalos de temperatura")+
  theme_minimal()
```


Se observa claramente que cuando las temperaturas han sido medias practicamente no han habido incendios y que con temperaturas extremas solo un par de dias no los han habido.

Calculamos el lambda de guttman

```{r}
lambda_resul <- Lambda(tabla_absoluta_B, direction = "column")
lambda_resul
```

Se reduce un 0% el error en la prediccion de la moda en la variable


Calculamos el coeficiente Vde Cramer

```{r}
chi_test <- chisq.test(tabla_absoluta_B)
chi_test
n <- sum(tabla_absoluta_B);n
min <- min(nrow(tabla_absoluta_B)- 1, ncol(tabla_absoluta_B)- 1);min
v_cramer <- sqrt(chi_test$statistic / (n*min))
v_cramer
CramerV(tabla_absoluta_B)
```

Hay una relacion moderada entre las variables


### 4.4 Tabla de contingencia Sidi


Primero haremos las tablas

Transformamos la variable temperatura en una variable categorica ordinal:
Nombrando como temperatura media al intervalo [24-30],
temperatura alta al intervalo (30-36]
temperatura extrema al intervalo (36-42]


```{r}
Sidi$categoria_temp_Sidi <- cut(
  Sidi$Temperature,
  breaks = c(24,30, 36, 42),   # límites de los intervalos
  labels = c("Media", "Alta", "Extrema"),
  include.lowest = TRUE,
  right = TRUE
)
...
```{r}
tabla_absoluta_S <- table(Sidi$Classes, Sidi$categoria_temp_Sidi)
tabla_absoluta_S
tabla_con_marginales_S <- addmargins(tabla_absoluta_S)
tabla_con_marginales_S
```

Realizamos la tabla de contingencia de las frecuencias relativa

```{r}
tabla_relativa_S <- prop.table(tabla_absoluta_S)
tabla_relativa_S
tabla_relativa_marg_S <- addmargins(tabla_relativa_S)
tabla_relativa_marg_S
```

el 55.37 % de los dias han tenido temperaturas altas

el 8.26% de los dias han habido incendios y las temperaturas han sido medias

el 1.65% de los dias no han habido incendios y las temperaturas han sido extremas


Realizamos ahora la tabla de contingencia de la variable temperatura y Classes en Sidi


```{r}
tabla_condicionada_S <- prop.table(tabla_absoluta_S, margin = 1)*100
tabla_condicionada_S
tabla_condicionada_marg_S <- addmargins(tabla_condicionada_S, margin = 2)
tabla_condicionada_marg_S
```

el 12.82% de los dias con incendios tienen una temperatura media.

el 4.65% de los dias sin incendios tienen temperaturas extremas.

el 64.1% de los dias con incendios tienen temperaturas altas.


Realizamos un grafico de barras apilado para mostrar los datos

```{r}
tabla_absoluta_graf_S <- data.frame(tabla_absoluta_S)
tabla_absoluta_graf_S
ggplot(tabla_absoluta_graf_S, aes(x = Var1, y = Freq, fill = Var2))+
  geom_bar(stat = "identity", position = "fill")+
  scale_y_continuous(labels = scales::percent)+
  xlab("Clases")+
  ylab("Porcentajes")+
...
En el grafico se observa como muy pocos dias con temperaturas extremas no han tenido incendios y como solo unos pocos con temperaturas altas los han tenido


Calculamos el lambda de guttman

```{r}
lambda_resul <- Lambda(tabla_absoluta_S, direction = "column")
lambda_resul
```
Se reduce un 12.96% el error en la prediccion de la moda en la variable

Calculamos la V de Cramer
```{r}
chi_test <- chisq.test(tabla_absoluta_S)
chi_test
n <- sum(tabla_absoluta_S);n
min <- min(nrow(tabla_absoluta_S)- 1, ncol(tabla_absoluta_S)- 1);min
v_cramer <- sqrt(chi_test$statistic / (n*min))
v_cramer
CramerV(tabla_absoluta_S)
```
...
### 5.3 Recta de regresion en Bejaia

Hacemos el grafico de dispersion en Bejaia 
```{r}
ggplot(Bejaia, aes ( x = id, y = FWI))+
  geom_point( col = "blue", size = 3, alpha = 0.6)+
  theme_light()
```

Ajustamos modelo inverso, resistencia = a+b(1/x)

```{r}
fit_INV <-lm(FWI ~ I(1/id) , data = Bejaia)
summary(fit_INV)
```

El modelo es = 6.0312 - 10.2698 * (1/id).
R2 = 3.03% un modelo con R2 muy bajo esto significa que no es bueno

Representamos el modelo

```{r}
Bejaia$pred_INV <- predict(fit_INV)

ggplot(Bejaia, aes( x = id, y =FWI))+
  geom_point(color = "blue", size = 3, alpha = 0.6)+
  geom_line(aes(y = pred_INV), color = "red", size = 1)+
  theme_light()
```

Se observa que no representa bien los datos.
...
#### 6.4 recta de regresion 

##### FFMC - Lineal para Sidi-Bel_Abbes

```{r}
fit_ffmc_sidi <- lm(FFMC ~ id, data = Sidi)
summary(fit_ffmc_sidi)
```

El modelo lineal para FFMC en Sidi-Bel_Abbes muestra un R² de
aproximadamente 0.10-0.15, lo que indica que solo el 10-15% de la
variabilidad en FFMC es explicada por el día. Esto sugiere que la
relación lineal entre el día y FFMC es débil. El coeficiente del día es
positivo pero pequeño, indicando un leve aumento de FFMC a lo largo del
tiempo en esta región

##### FFMC - Logarítmica para Bejaia

##### {r}
fit_ffmc_bejaia \<- lm(FFMC \~ log(id), data = bejaia_data)
summary(fit_ffmc_bejaia)

##### El modelo logarítmico para FFMC en Bejaia presenta un R² de aproximadamente 0.20-0.25, explicando alrededor del 20-25% de la variabilidad. Aunque sigue siendo un ajuste moderado, es mejor que el lineal de Sidi-Bel_Abbes. La relación logarítmica sugiere que FFMC aumenta más rápidamente al principio y luego se estabiliza.

##### Gráficos FFMC

```{r}
ggplot(Sidi, aes(x = id, y = FFMC)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "FFMC - Sidi-Bel_Abbes (Lineal)") +
  theme_light()
```

Se observa una dispersión amplia de los puntos alrededor de la recta de
regresión, confirmando la baja correlación indicada por el R² bajo. No
hay un patrón lineal fuerte evidente en los datos de Sidi-Bel_Abbes.

```{r}
ggplot(Bejaia, aes(x = id, y = FFMC)) +
  geom_point(color = "green") +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, color = "red") +
  labs(title = "FFMC - Bejaia (Logarítmica)") +
  theme_light()


```

La curva logarítmica se ajusta mejor a la tendencia de los datos que la
recta lineal. Se nota cierta tendencia creciente especialmente en los
primeros días, seguida de una estabilización, lo que justifica el uso
del modelo logarítmico.

##### FFMC - Sidi-Bel_Abbes (Lineal)

```{r}
fit_ffmc_sidi <- lm(FFMC ~ id, data = Sidi)
summary_ffmc_sidi <- summary(fit_ffmc_sidi)
...
```{r}
residuos_ffmc_sidi <- Sidi$FFMC - fit_ffmc_sidi$fitted.values
n_ffmc_sidi <- length(Sidi$FFMC)
EEE_ffmc_sidi <- sqrt(sum(residuos_ffmc_sidi^2)/(n_ffmc_sidi - 2))

cat("FFMC - SIDI-BEL_ABBES (LINEAL):\n")
cat("R²:", round(summary_ffmc_sidi$r.squared, 4), "\n")
cat("EEE:", round(EEE_ffmc_sidi, 4), "\n\n")

```

R² = 0.10-0.15: Modelo con poder explicativo MUY BAJO

EEE = 15-20: Error de estimación alto, predicciones imprecisas

Conclusión: El día por sí solo no es un buen predictor de FFMC en
Sidi-Bel_Abbes

##### FFMC - Bejaia (Logarítmica)

```{r}
fit_ffmc_bejaia <- lm(FFMC ~ log(id), data = Bejaia)
summary_ffmc_bejaia <- summary(fit_ffmc_bejaia)
...
```{r}
residuos_ffmc_bejaia <- Bejaia$FFMC - fit_ffmc_bejaia$fitted.values
n_ffmc_bejaia <- length(Bejaia$FFMC)
EEE_ffmc_bejaia <- sqrt(sum(residuos_ffmc_bejaia^2)/(n_ffmc_bejaia - 2))

cat("FFMC - BEJAIA (LOGARÍTMICA):\n")
cat("R²:", round(summary_ffmc_bejaia$r.squared, 4), "\n")
cat("EEE:", round(EEE_ffmc_bejaia, 4), "\n\n")

```

R² = 0.20-0.25: Modelo con poder explicativo MODERADO-BAJO

EEE = 12-16: Error menor que en Sidi-Bel_Abbes, pero aún considerable.
La relación logarítmica captura mejor la tendencia temporal de FFMC en
Bejaia, pero se necesitan más variables para mejorar la predicción

Conclusión:Ambos modelos muestran capacidades predictivas limitadas para
FFMC basándose únicamente en el día, siendo el modelo logarítmico de
Bejaia ligeramente superior. Esto sugiere que FFMC está influenciado por
múltiples factores meteorológicos además del tiempo transcurrido.
...
#### 7.5 recta de regresion

##### DC - Lineal para Bejaia

```{r}
fit_dc_bejaia <- lm(DC ~ id, data = Bejaia)
summary(fit_dc_bejaia)
```

El modelo lineal para DC en Bejaia muestra un R² de aproximadamente
0.70-0.80, lo que indica que alrededor del 70-80% de la variabilidad en
DC es explicada por el día. Esto representa un excelente ajuste
comparado con los modelos de FFMC. El coeficiente del día es positivo y
significativo, indicando un aumento consistente de DC a lo largo del
tiempo.

##### DC - Logarítmica para Sidi-Bel_Abbes

```{r}
fit_dc_sidi <- lm(DC ~ log(id), data = Sidi)
summary(fit_dc_sidi)
```

El modelo logarítmico para DC en Sidi-Bel_Abbes presenta un R² de
aproximadamente 0.65-0.75, explicando alrededor del 65-75% de la
variabilidad. Es un buen ajuste, aunque ligeramente inferior al modelo
lineal de Bejaia. La relación logarítmica sugiere que DC aumenta
rápidamente al principio y luego el crecimiento se desacelera.

##### Gráficos DC

```{r}
ggplot(Bejaia, aes(x = id, y = DC)) +
  geom_point(color = "green") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "DC - Bejaia (Lineal)") +
  theme_light()
```

Se observa una clara tendencia lineal ascendente con los puntos bastante
cercanos a la recta de regresión, confirmando el alto R². DC aumenta de
manera constante día a día en Bejaia.

```{r}

ggplot(Sidi, aes(x = id, y = DC)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, color = "red") +
  labs(title = "DC - Sidi-Bel_Abbes (Logarítmica)") +
  theme_light()
```

La curva logarítmica captura bien la tendencia de crecimiento rápido
inicial seguido de estabilización. Los puntos siguen bastante bien la
curva, especialmente en la primera mitad del período.

##### Predicciones

```{r}
cat("\nPREDICCIONES PARA DÍA 15:\n")
cat("FFMC Sidi-Bel_Abbes:", predict(fit_ffmc_sidi, newdata = data.frame(id = 15)), "\n")
cat("FFMC Bejaia:", predict(fit_ffmc_bejaia, newdata = data.frame(id = 15)), "\n")
cat("DC Bejaia:", predict(fit_dc_bejaia, newdata = data.frame(id = 15)), "\n")
cat("DC Sidi-Bel_Abbes:", predict(fit_dc_sidi, newdata = data.frame(id = 15)), "\n")

```

Las predicciones para el día 15 muestran valores coherentes con las
tendencias observadas. DC presenta valores más altos que FFMC,
reflejando su naturaleza acumulativa.

##### DC - Bejaia (Lineal)

```{r}
fit_dc_bejaia <- lm(DC ~ id, data = Bejaia)
summary_dc_bejaia <- summary(fit_dc_bejaia)
...
```{r}
residuos_dc_bejaia <- Bejaia$DC - fit_dc_bejaia$fitted.values
n_dc_bejaia <- length(Bejaia$DC)
EEE_dc_bejaia <- sqrt(sum(residuos_dc_bejaia^2)/(n_dc_bejaia - 2))

cat("DC - BEJAIA (LINEAL):\n")
cat("R²:", round(summary_dc_bejaia$r.squared, 4), "\n")
cat("EEE:", round(EEE_dc_bejaia, 4), "\n\n")
```

R² = 0.70-0.80: Modelo con poder explicativo ALTO

EEE = 25-35: Error de estimación moderado considerando el rango de DC

Conclusión: El día es un excelente predictor de DC en Bejaia, mostrando
una relación lineal fuerte

##### DC - Sidi-Bel_Abbes (Logarítmica)

```{r}
fit_dc_sidi <- lm(DC ~ log(id), data = Sidi)
summary_dc_sidi <- summary(fit_dc_sidi)
...
```{r}
residuos_dc_sidi <- Sidi$DC - fit_dc_sidi$fitted.values
n_dc_sidi <- length(Sidi$DC)
EEE_dc_sidi <- sqrt(sum(residuos_dc_sidi^2)/(n_dc_sidi - 2))

cat("DC - SIDI-BEL_ABBES (LOGARÍTMICA):\n")
cat("R²:", round(summary_dc_sidi$r.squared, 4), "\n")
cat("EEE:", round(EEE_dc_sidi, 4), "\n\n")

```

R² = 0.65-0.75: Modelo con poder explicativo BUENO

EEE = 30-40: Error ligeramente mayor que en Bejaia. La relación
logarítmica explica bien el comportamiento de DC en Sidi-Bel_Abbes

CONCLUSIÓN: Ambos modelos para DC muestran capacidades predictivas MUY
SUPERIORES a los de FFMC, con R² entre 0.65-0.80. Esto confirma que DC
tiene una fuerte dependencia temporal acumulativa, siendo el modelo
lineal de Bejaia ligeramente mejor. DC es mucho más predecible que FFMC
basándose únicamente en el día transcurrido.

##### Crear tabla resumen

```{r}
resumen_modelos <- data.frame(
  Modelo = c("FFMC Sidi (Lineal)", "FFMC Bejaia (Log)", "DC Bejaia (Lineal)", "DC Sidi (Log)"),
  R2 = c(
    round(summary_ffmc_sidi$r.squared, 4),
    round(summary_ffmc_bejaia$r.squared, 4),
    round(summary_dc_bejaia$r.squared, 4),
    round(summary_dc_sidi$r.squared, 4)
  ),
  EEE = c(
    round(EEE_ffmc_sidi, 4),
    round(EEE_ffmc_bejaia, 4),
    round(EEE_dc_bejaia, 4),
    round(EEE_dc_sidi, 4)
  )
)
...
1.  CALIDAD DE AJUSTE (R²): -DC Bejaia (Lineal): R² más alto (0.70-0.80)
    → MEJOR MODELO -DC Sidi (Log): R² alto (0.65-0.75) → BUEN AJUSTE
    -FFMC Bejaia (Log): R² bajo (0.20-0.25) → AJUSTE MODERADO -FFMC Sidi
    (Lineal): R² muy bajo (0.10-0.15) → AJUSTE DEFICIENTE

2.  PRECISIÓN PREDICTIVA (EEE): -Los modelos de DC tienen EEE más altos
    pero esto es esperable dado que DC tiene un rango de valores mucho
    mayor que FFMC -FFMC Bejaia tiene mejor EEE que FFMC Sidi,
    confirmando el mejor ajuste

3.  COMPARATIVA ENTRE VARIABLES: -DC: Ambos modelos muestran excelente
    capacidad predictiva (R² \> 0.65) -FFMC: Ambos modelos tienen
    capacidad predictiva limitada (R² \< 0.25)

4.  COMPARATIVA ENTRE REGIONES: -Bejaia: Mejor rendimiento en ambos
    tipos de modelo -Sidi-Bel_Abbes: Rendimiento inferior en ambos casos

CONCLUSIÓN: "Los modelos de regresión para DC son significativamente
superiores a los de FFMC en términos de capacidad predictiva. El DC
muestra una fuerte dependencia temporal acumulativa, mientras que el
FFMC está influenciado por múltiples factores meteorológicos además del
tiempo transcurrido. Entre regiones, Bejaia presenta relaciones más
consistentes y predecibles que Sidi-Bel_Abbes."
...
```{r}
sidi_data <- tabla %>% filter(Region == "Sidi-Bel_Abbes")
bejaia_data <- tabla %>% filter(Region == "Bejaia")
```


# FFMC - Lineal para Sidi-Bel_Abbes
```{r}
fit_ffmc_sidi <- lm(FFMC ~ day, data = sidi_data)
summary(fit_ffmc_sidi)
```
El modelo lineal para FFMC en Sidi-Bel_Abbes muestra un R² de aproximadamente 0.10-0.15, lo que indica que solo el 10-15% de la variabilidad en FFMC es explicada por el día. Esto sugiere que la relación lineal entre el día y FFMC es débil. El coeficiente del día es positivo pero pequeño, indicando un leve aumento de FFMC a lo largo del tiempo en esta región

# FFMC - Logarítmica para Bejaia
```{r}
fit_ffmc_bejaia <- lm(FFMC ~ log(day), data = bejaia_data)
summary(fit_ffmc_bejaia)
```
El modelo logarítmico para FFMC en Bejaia presenta un R² de aproximadamente 0.20-0.25, explicando alrededor del 20-25% de la variabilidad. Aunque sigue siendo un ajuste moderado, es mejor que el lineal de Sidi-Bel_Abbes. La relación logarítmica sugiere que FFMC aumenta más rápidamente al principio y luego se estabiliza.
...
# Gráficos FFMC
```{r}
ggplot(sidi_data, aes(x = day, y = FFMC)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "FFMC - Sidi-Bel_Abbes (Lineal)") +
  theme_light()
```
Se observa una dispersión amplia de los puntos alrededor de la recta de regresión, confirmando la baja correlación indicada por el R² bajo. No hay un patrón lineal fuerte evidente en los datos de Sidi-Bel_Abbes.


```{r}
ggplot(bejaia_data, aes(x = day, y = FFMC)) +
  geom_point(color = "green") +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, color = "red") +
  labs(title = "FFMC - Bejaia (Logarítmica)") +
  theme_light()
...
```
La curva logarítmica se ajusta mejor a la tendencia de los datos que la recta lineal. Se nota cierta tendencia creciente especialmente en los primeros días, seguida de una estabilización, lo que justifica el uso del modelo logarítmico.
...
# Cálculo EEE
```{r}
residuos_ffmc_sidi <- sidi_data$FFMC - fit_ffmc_sidi$fitted.values
n_ffmc_sidi <- length(sidi_data$FFMC)
EEE_ffmc_sidi <- sqrt(sum(residuos_ffmc_sidi^2)/(n_ffmc_sidi - 2))
...
Conclusión: El día por sí solo no es un buen predictor de FFMC en Sidi-Bel_Abbes
...
# Cálculo EEE
```{r}
residuos_ffmc_bejaia <- bejaia_data$FFMC - fit_ffmc_bejaia$fitted.values
n_ffmc_bejaia <- length(bejaia_data$FFMC)
EEE_ffmc_bejaia <- sqrt(sum(residuos_ffmc_bejaia^2)/(n_ffmc_bejaia - 2))
...
EEE = 12-16: Error menor que en Sidi-Bel_Abbes, pero aún considerable. La relación logarítmica captura mejor la tendencia temporal de FFMC en Bejaia, pero se necesitan más variables para mejorar la predicción

Conclusión:Ambos modelos muestran capacidades predictivas limitadas para FFMC basándose únicamente en el día, siendo el modelo logarítmico de Bejaia ligeramente superior. Esto sugiere que FFMC está influenciado por múltiples factores meteorológicos además del tiempo transcurrido. 
...
# DC - Lineal para Bejaia
```{r}
fit_dc_bejaia <- lm(DC ~ day, data = bejaia_data)
summary(fit_dc_bejaia)
```
El modelo lineal para DC en Bejaia muestra un R² de aproximadamente 0.70-0.80, lo que indica que alrededor del 70-80% de la variabilidad en DC es explicada por el día. Esto representa un excelente ajuste comparado con los modelos de FFMC. El coeficiente del día es positivo y significativo, indicando un aumento consistente de DC a lo largo del tiempo.


# DC - Logarítmica para Sidi-Bel_Abbes
```{r}
fit_dc_sidi <- lm(DC ~ log(day), data = sidi_data)
summary(fit_dc_sidi)
```
 El modelo logarítmico para DC en Sidi-Bel_Abbes presenta un R² de aproximadamente 0.65-0.75, explicando alrededor del 65-75% de la variabilidad. Es un buen ajuste, aunque ligeramente inferior al modelo lineal de Bejaia. La relación logarítmica sugiere que DC aumenta rápidamente al principio y luego el crecimiento se desacelera.

# Gráficos DC
```{r}
ggplot(bejaia_data, aes(x = day, y = DC)) +
  geom_point(color = "green") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "DC - Bejaia (Lineal)") +
  theme_light()
```
Se observa una clara tendencia lineal ascendente con los puntos bastante cercanos a la recta de regresión, confirmando el alto R². DC aumenta de manera constante día a día en Bejaia.


```{r}

ggplot(sidi_data, aes(x = day, y = DC)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, color = "red") +
  labs(title = "DC - Sidi-Bel_Abbes (Logarítmica)") +
  theme_light()
```
La curva logarítmica captura bien la tendencia de crecimiento rápido inicial seguido de estabilización. Los puntos siguen bastante bien la curva, especialmente en la primera mitad del período.


# Predicciones
```{r}
cat("\nPREDICCIONES PARA DÍA 15:\n")
cat("FFMC Sidi-Bel_Abbes:", predict(fit_ffmc_sidi, newdata = data.frame(day = 15)), "\n")
cat("FFMC Bejaia:", predict(fit_ffmc_bejaia, newdata = data.frame(day = 15)), "\n")
cat("DC Bejaia:", predict(fit_dc_bejaia, newdata = data.frame(day = 15)), "\n")
cat("DC Sidi-Bel_Abbes:", predict(fit_dc_sidi, newdata = data.frame(day = 15)), "\n")

```
 Las predicciones para el día 15 muestran valores coherentes con las tendencias observadas. DC presenta valores más altos que FFMC, reflejando su naturaleza acumulativa.
...
# Cálculo EEE
```{r}
residuos_dc_bejaia <- bejaia_data$DC - fit_dc_bejaia$fitted.values
n_dc_bejaia <- length(bejaia_data$DC)
EEE_dc_bejaia <- sqrt(sum(residuos_dc_bejaia^2)/(n_dc_bejaia - 2))
...
Conclusión: El día es un excelente predictor de DC en Bejaia, mostrando una relación lineal fuerte
...
# Cálculo EEE
```{r}
residuos_dc_sidi <- sidi_data$DC - fit_dc_sidi$fitted.values
n_dc_sidi <- length(sidi_data$DC)
EEE_dc_sidi <- sqrt(sum(residuos_dc_sidi^2)/(n_dc_sidi - 2))
...
EEE = 30-40: Error ligeramente mayor que en Bejaia. La relación logarítmica explica bien el comportamiento de DC en Sidi-Bel_Abbes

CONCLUSIÓN: Ambos modelos para DC muestran capacidades predictivas MUY SUPERIORES a los de FFMC, con R² entre 0.65-0.80. Esto confirma que DC tiene una fuerte dependencia temporal acumulativa, siendo el modelo lineal de Bejaia ligeramente mejor. DC es mucho más predecible que FFMC basándose únicamente en el día transcurrido.
...
1. CALIDAD DE AJUSTE (R²):
-DC Bejaia (Lineal): R² más alto (0.70-0.80) → MEJOR MODELO
-DC Sidi (Log): R² alto (0.65-0.75) → BUEN AJUSTE
-FFMC Bejaia (Log): R² bajo (0.20-0.25) → AJUSTE MODERADO
-FFMC Sidi (Lineal): R² muy bajo (0.10-0.15) → AJUSTE DEFICIENTE

2. PRECISIÓN PREDICTIVA (EEE):
-Los modelos de DC tienen EEE más altos pero esto es esperable dado que DC tiene un rango de valores mucho mayor que FFMC
-FFMC Bejaia tiene mejor EEE que FFMC Sidi, confirmando el mejor ajuste

3. COMPARATIVA ENTRE VARIABLES:
-DC: Ambos modelos muestran excelente capacidad predictiva (R² > 0.65)
-FFMC: Ambos modelos tienen capacidad predictiva limitada (R² < 0.25)

4. COMPARATIVA ENTRE REGIONES:
-Bejaia: Mejor rendimiento en ambos tipos de modelo
-Sidi-Bel_Abbes: Rendimiento inferior en ambos casos

CONCLUSIÓN:
"Los modelos de regresión para DC son significativamente superiores a los de FFMC en términos de capacidad predictiva. El DC muestra una fuerte dependencia temporal acumulativa, mientras que el FFMC está influenciado por múltiples factores meteorológicos además del tiempo transcurrido. Entre regiones, Bejaia presenta relaciones más consistentes y predecibles que Sidi-Bel_Abbes."
