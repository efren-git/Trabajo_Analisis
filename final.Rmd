...
Ajustamos modelo logaritmico, resistencia = a+b LN(id)

{r}
fit_LNX <-lm(FWI ~ log(id) , data = Bejaia)
summary(fit_LNX)


El modelo es = -0.03608 + 1.46531 * LN(id).
R2 = 4.68% un modelo con R2 muy bajo esto significa que no es bueno

Representamos el modelo

{r}
Bejaia$pred_LNX <- predict(fit_LNX)
ggplot(Bejaia, aes( x = id, y =FWI))+
  geom_point(color = "blue", size = 3, alpha = 0.6)+
  geom_line(aes(y = pred_LNX), color = "red", size = 1)+
  theme_light()
...
```{r}
fit_ffmc_sidi <- lm(FFMC ~ id, data = sidi_data)
...
# Gráficos FFMC
```{r}
ggplot(sidi_data, aes(x = id, y = FFMC)) +
...
```{r}
ggplot(bejaia_data, aes(x = id, y = FFMC)) +
...
```{r}
fit_dc_bejaia <- lm(DC ~ id, data = bejaia_data)
...
```{r}
fit_dc_sidi <- lm(DC ~ log(id), data = sidi_data)
summary(fit_dc_sidi)
```
...
# Gráficos DC
```{r}
ggplot(bejaia_data, aes(x = id, y = DC)) +
  geom_point(color = "green") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
ggplot(sidi_data, aes(x = id, y = DC)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", formula = y ~ log(x), se = FALSE, color = "red") +
